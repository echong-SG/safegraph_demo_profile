...
def get_raw_census_data(demos_to_analyze, open_census_data_dir, census_year, drive=None, verbose=False):
    ...
    # These are the supported options for table_id strings: 
    #    'Sex By Age', 
    #    'Race', 
    ...
    prefixes = set(get_census_prefix(demos_to_analyze, cbg_field_desc, census_year) + ['b01']) # 'b01' we need for total_population
...
def get_home_panel(patterns_dir=None,  drive=None):
    if(patterns_dir):
        home_panel = pd.read_csv(os.path.join(patterns_dir, "home_panel_summary.csv"), dtype = {'census_block_group': str}).drop(['year', 'month','region'],axis='columns')
    elif(drive):
        home_panel = pd_read_csv_drive(get_drive_id('home_panel_summary.csv'), drive, dtype = {'census_block_group': str}).drop(['year', 'month','region'],axis='columns')
